...
public class App extends ListenerAdapter implements EventListener {
	...
	/**
	 * This method will get the message if it starts with our prefix and extracts the command.
	 */
	@Override
	public void onMessageReceived(MessageReceivedEvent event) {
		if(event.getMessage().getContentRaw().startsWith(Config.getPrefix()) && !event.getAuthor().isBot()) {
			...
			try {
				...
				event.getChannel().sendMessage("`" + plugin + "` is not a feature, try " + Config.getPrefix() + "help").queue();
			} catch (Exception e2) {
